...
.loop_j:
    ...
    ; Просто суммируем сырые значения в res_buf без промежуточных переносов
...
.normalize_carries:
    ; ФИКС: Проходим по всему результирующему массиву один раз 
    ; и корректно разносим все скопившиеся переносы десятков
    xor REG_SI, REG_SI
    xor REG_C, REG_C       ; REG_C = текущий перенос (carry)
.carry_loop:
    movzx REG_A, byte [res_buf + REG_SI]
    add REG_A, REG_C       ; Добавляем прошлый перенос
    
    test REG_A, REG_A
    jnz .do_div
    cmp REG_SI, [len1_32]  ; Если число кончилось и переноса нет — выходим
    jl .zero_digit
    jmp .invert_res
.zero_digit:
    mov byte [res_buf + REG_SI], 0
    mov REG_C, 0
    inc REG_SI
    jmp .carry_loop
...
.do_div:
    ...
    push REG_SI
    xor REG_D, REG_D
    div REG_B              ; REG_A = частное (новый перенос), REG_D = остаток (цифра)
    mov REG_C, REG_A       
    mov REG_A, REG_D       
    pop REG_SI
...
.invert_res:
    ; Переводим все цифры строго один раз в ASCII и ищем реальный конец строки
    xor REG_SI, REG_SI
.find_end:
...
.check_zero:
    test REG_SI, REG_SI
    jnz .do_invert
    ; Если результат вообще пустой, значит это чистый ноль
    mov byte [res_buf], '0'
